readerobj = VideoReader('shahar_walk.avi', 'tag', 'myreader1');
...
n=n-1;
e_x=zeros(3,12);
eigen_x=zeros(numFrames-1,3);
data=zeros(numFrames-1,5);
STRAIN_ENERGY=zeros(numFrames-1,1);
POTENTIAL_ENERGY=zeros(numFrames-1,1);
...
e_x=[ e_x(1,1) e_x(1,2) e_x(1,3); e_x(2,1) e_x(2,2) e_x(2,3);e_x(3,1) e_x(3,2) e_x(3,3)];
% eigen=size(1,numFrames);
eigen=eig(e_x);
eigen_x(g,:)=eigen;
%disp(eigen_x)
%figure,plot(eigen);
...
m=m+1;
data=[eigen_x,STRAIN_ENERGY,POTENTIAL_ENERGY];%matrix of features data set
end
%%%%%%%%%%%%%%%%%%%%%%%%% WRITING EXCEL SHEET
%a=xlswrite('aa.xls',data,1,'B2:F21');

[text]=xlswrite('aa.xls',[{'shahar_walk'}],1,'A881:A881');
a=xlswrite('aa.xls',data,1,'B881:F900');
...
%    n=20;
% rand=randperm(n);
% randomsample=training(rand(1:pointstopick),:);
% class = knnclassify(randomsample, training, traininglabel, 5);
% %cp = classperf(traininglabel,class);
% %a1=cp.correctrate;
% b1=training-randomsample;
% class1 = knnclassify(b1, training, traininglabel, 5);
% cp = classperf(traininglabel,class1);
% c1=cp.correctrate;
% d1=(a1+c1)/2;
